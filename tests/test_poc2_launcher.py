...
with tempfile.TemporaryDirectory() as tmp:
 root=Path(tmp);home=root/'home/pcbm';(home/'content').mkdir(parents=True);(root/'usr/bin').mkdir(parents=True)
 script=root/'run';script.write_text(src.replace('/home/pcbm',str(home)).replace('/usr/bin/',str(root/'usr/bin')+'/').replace('/usr/libexec/',str(root/'usr/libexec')+'/').replace('/etc/pcbm/',str(root/'etc/pcbm')+'/'))
 ...
 media=home/'content/test content.prg';media.write_text('owned test')
