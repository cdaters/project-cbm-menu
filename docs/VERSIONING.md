# Project CBM Versioning

Project CBM uses two version tracks:

```text
PCBM_VERSION      = public SD-card image/build version
PCBM_MENU_VERSION = private menu system version
```

For the first public image:

```text
Project CBM image: v1.0.0
Project CBM menu:  v6.5
```

This lets the private menu system evolve without forcing a public image release every time a script or build-note detail changes.
